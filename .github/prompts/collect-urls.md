...
収集対象ソース・処理ルール・出力ファイル仕様・フィールド定義は **GEMINI.md の「ニュース収集の要件」** を参照してください。

---

## 手順 1. RSSフィードの取得

GEMINI.md の「収集対象ソース」に記載された各RSSフィード（各最大10件）を以下の優先順位で取得する。

### 1-A. WebFetch による直接取得

GEMINI.md の RSS URL を WebFetch で取得する。成功した場合はそのまま記事を抽出する。
